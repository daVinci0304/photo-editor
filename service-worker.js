...
<body>
    ...
    <script>
        ...
        // --- 공유 기능을 위한 '보관함(Cache)' 방식 최종 로직 ---

        // 1. '보관함'을 확인해서 공유된 이미지가 있는지 확인하는 함수
        async function checkForSharedImage() {
            try {
                const cache = await caches.open('shared-image-cache-v1');
                ...
                if (response) {
                    ...
                    // 중요: 사용한 이미지는 보관함에서 삭제하여 다음 실행에 영향을 주지 않도록 합니다.
        ...
        // 2. 서비스 워커 등록 및 보관함 확인 실행
        if ('serviceWorker' in navigator) {
            window.addEventListener('load', () => {
                navigator.serviceWorker.register('./service-worker.js')
                    .then(registration => {
                        console.log('ServiceWorker registration successful');
                        // 서비스 워커가 활성화되면 보관함을 확인합니다.
                        if (navigator.serviceWorker.controller) {
                            checkForSharedImage();
                        }
                        // 서비스 워커가 바뀌었을 때도 확인합니다.
